function [Phi_star, Sigma_star, E_xx] = DSGE_VAR_2(theta,p)
...
Psi0_temp = [ Psi0(1); Psi0(3)];     %gamma(growth), inflation
Psi0 = Psi0_temp;
m = length(Psi0);    %Number of variables in the VAR


gamma_0 = DSGEautocovarIRF_2(theta,0);     %autocovariance of s_t 에서 diagonal(no lag) 
...
for i = 1:p
    ...
    for j = i:p
        h = abs(i-j);
        gamma_h = DSGEautocovarIRF_2(theta,h);
...
E_xx = [1, repmat(Psi0,p,1)'; repmat(Psi0,p,1), E_xx];
%E_xx = [E_xx, repmat(Psi0,p,1); repmat(Psi0,p,1)', 1];


gamma_1 = DSGEautocovarIRF_2(theta,1);
