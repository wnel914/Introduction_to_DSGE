...
%Construct coefficient matrices(19p)
bbeta = theta(1);
ggamma = theta(2);
llambda = theta(3);
piStar = theta(4);
zetaP = theta(5);
nu = theta(6);
rhoPhi = theta(7);     %persistency of shock 
rhoLambda = theta(8);
rhoZ = theta(9);
sigmaPhi = theta(10);     % std of shock
...
Phi1(5,:) = [-(kappa_P*psi_P/bbeta)/(1-psi_P*rhoPhi); ...
    ...
    - psi_P*sigmaR; 0]';     %x_phi, x_lambda, x_z, x_epsR, 0 : 5번째 row of Phi1
