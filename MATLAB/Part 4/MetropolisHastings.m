...
% Starting values
Thetasim      = zeros(Nsim,length(param_mode));     %Nsim(simulation 횟수)*theta 
Thetasim(1,:) = draw_prior;    %make initial draw from prior
%Thetasim(1,:) = param_mode;         %make initial draw from mode

% further initializations 
accept        = 0;
obj           = dsgeliki(Thetasim(1,:)) + prior(Thetasim(1,:));     %posterior density of each theta 
counter       = 0;
logposterior  = obj*ones(Nsim,1); %sum of posterior density induced from the initial theta 


for i=1:Nsim
    
    % generate proposal draw     c가 붙은 건 그 다음 draw(i+1) 
    Thetac = mvnrnd(Thetasim(i,:),c*Sigma);      %mean은 Thetasim(theha_i), variance는 c*Sigma인 theta_i+1(length : 1*13)를 추출 
    prioc  = prior(Thetac);     %거기서 또 simulation해서 (Y를?) draw
...
[Nsim,Npam] = size(Thetasim);      %Nsim(simulation 횟수)*theta 즉 Npam 은 theta의 개수(13) 
...
%hold off     %off로 해두면 다음 창 열릴 때 기존 창 닫힘
